...
use std::ops::Deref;
use dioxus::core::{Attribute, AttributeBox, AttributeValue, HasAttributesMut};
use dioxus::core::exports::bumpalo::Bump;
use dioxus::html::{ExtendedAudioMarker, ExtendedGlobalAttributesMarker};
use dioxus::prelude::*;
...
impl<'a, A> HasAttributesMut<'a, FooPropsBuilder<'a, (A, )>> for FooPropsBuilder<'a, (A, )> {
...
pub fn Foo<'a>(cx: Scope<'a, FooProps<'a>>) -> Element<'a> {
    ...
    static TEMPLATE: ::dioxus::core::Template = ::dioxus::core::Template { name: concat!(file!(), ":", line!(), ":", column!(), ":", "" ), roots: &[::dioxus::core::TemplateNode::Element { tag: dioxus_elements::audio::TAG_NAME, namespace: dioxus_elements::audio::NAME_SPACE, attrs: &[::dioxus::core::TemplateAttribute::Dynamic { id: 0usize }], children: &[] }], node_paths: &[], attr_paths: &[&[0u8]] };
            let mut attrs: Vec<Attribute> = vec![__cx.attr(dioxus_elements::audio::muted.0, muted,
                                                    dioxus_elements::audio::muted.1, dioxus_elements::audio::muted.2)];

            for attr in attributes {
                __cx.attr(attr.name, attr.clone().value.into_value(__cx.bump()), attr.namespace, attr.volatile);
            };
            ::dioxus::core::VNode {
        parent: None,
        key: None,
